...
extern AppStateManager* gManager;

#define USER_TIMEOUT 1.5
#define handPositionBufferLength 1
extern std::vector<Ogre::String> StateNames;
Ogre::OverlayElement* gRegionGuide;
extern FlashControl* gRegionFlash;
#define USE_CALIBRATION_FILE 0
XnPoint3D gHeadPosition={-1,-1,-1};
void KinectController::userSeen(const XnUserID nUserId)
{
	
	candidateUsers.push_back(nUserId);
	candidateUserInterestRate.push_back(0);
	XnPoint3D tempCOM;
	m_UserGenerator.GetCoM(nUserId,tempCOM);
	candidateUserCOMs.push_back(tempCOM);
}

bool KinectController::checkInterestRate(XnUserID user)
{

	XnPoint3D tempCOM,oldCOM;
	m_UserGenerator.GetCoM(user,tempCOM);
	if (tempCOM.Z<=0)
	{
		return false;
	}
	...
	{
		unsigned int i;
		for (i=0;i<candidateUsers.size();i++)
		{
			if (candidateUsers.at(i)==user)
			{
				oldCOM=candidateUserCOMs.at(i);
				break;
			}
		}
		float dist=sqrt(pow(oldCOM.X-tempCOM.X,2)+pow(oldCOM.Y-tempCOM.Y,2)+pow(oldCOM.Z-tempCOM.Z,2));
		candidateUserCOMs.at(i)=tempCOM;
		if (dist<20)
			candidateUserInterestRate.at(i)++;

		if (candidateUserInterestRate.at(i)>1)
		{
			candidateUserInterestRate.at(i)=0;
			return true;
		}
		else
			return false;
	...
	void XN_CALLBACK_TYPE KinectController::NewUser(xn::UserGenerator& generator, const XnUserID nUserId, void* pCookie)
	{
		...
		OgreFramework::getSingletonPtr()->m_pLog->logMessage("New user entered the scene:" + Ogre::StringConverter::toString(nUserId) );
		KinectController* This = (KinectController*)pCookie;
		This->userSeen(nUserId);
	}
	...
	void XN_CALLBACK_TYPE KinectController::UserExit(xn::UserGenerator& generator, const XnUserID nUserId, void* pCookie)
	{
		...
		for (std::vector<XnUserID>::iterator i=This->candidateUsers.begin();i!=This->candidateUsers.end();i++)
		{
			if (*i._Ptr==nUserId)
			{
				OgreFramework::getSingletonPtr()->m_pLog->logMessage("Interested user moved away, deleting");
				if (*i._Ptr==This->interestedUser)
					This->interestedUser=0;
				This->candidateUsers.erase(i);
				This->candidateUserCOMs.erase(This->candidateUserCOMs.begin()+index);
				This->candidateUserInterestRate.erase(This->candidateUserInterestRate.begin()+index);
				break;
			}
			index++;
		}
	...
	void XN_CALLBACK_TYPE KinectController::UserReEnter(xn::UserGenerator& generator, const XnUserID nUserId, void* pCookie)
	{
		// start looking for calibration pose for new users
		//generator.GetPoseDetectionCap().StartPoseDetection("Psi", nUserId);
		OgreFramework::getSingletonPtr()->m_pLog->logMessage("A new user reentered the scene");
		KinectController* This = (KinectController*)pCookie;
		This->userSeen(nUserId);
	}
	...
	void XN_CALLBACK_TYPE KinectController::LostUser(xn::UserGenerator& generator, const XnUserID nUserId, void* pCookie)
	{
		...
		{
			OgreFramework::getSingletonPtr()->m_pLog->logMessage("Active user was lost from the sensor, this is very extraordinary");
	...
	void XN_CALLBACK_TYPE KinectController::CalibrationEnd(xn::SkeletonCapability& skeleton, const XnUserID nUserId, XnBool bSuccess, void* pCookie)
	{
		...
		{
			// start tracking
			OgreFramework::getSingletonPtr()->m_pLog->logMessage("Completed a calibration successfully, initiating sub processes");
...
//			This->m_pQuitFlow->SetActive(NULL);
			...
			OgreFramework::getSingletonPtr()->m_pLog->logMessage("Completed post-calibration processes");
		}
		else
		{
			OgreFramework::getSingletonPtr()->m_pLog->logMessage("Unsuccessful calibration, resetting pose detection");
	...
	void XN_CALLBACK_TYPE KinectController::PoseDetected(xn::PoseDetectionCapability& poseDetection, const XnChar* strPose, XnUserID nId, void* pCookie)
	{
		...
		if(This->activeUser != 0)
		{
			...
			OgreFramework::getSingletonPtr()->m_pLog->logMessage("Detected a pose");
	...
	{
		OgreFramework::getSingletonPtr()->m_pLog->logMessage("Lost a pose");
		KinectController* This = (KinectController*)pCookie;
		This->m_pStartPoseDetector->Reset();
	}

	KinectController::KinectController(bool useHandTracker,StatisticsManager* stats):
	...
	XnStatus KinectController::initPrimeSensor(bool useHandTracker)
	{
		...
		skel.SetSkeletonProfile(XN_SKEL_PROFILE_UPPER);
	...
	bool KinectController::addTime(Real deltaTime,bool transitionInProgress)
	{
		...
		{
			OgreFramework::getSingletonPtr()->m_pLog->logMessage("Exception Occured during Kinect Update");
			const char* c=e.what();
		}
		...
		if (m_DepthGenerator.IsDataNew())
		{
			if (!transitionInProgress)
			{
				if (candidateUsers.size()==0)//THis means there are no users seen in the scene whatsoever
				{
					...
					activeUser=0;
					interestedUser=0;
					if (gManager->getActiveStateStack()->size()>0)
					{
						if (gManager->getActiveStateStack()->back()->m_Index!=StandByStateId)
						{
							OgreFramework::getSingletonPtr()->m_pLog->logMessage("Passed onto stand by State with no users in the scene");
							gManager->popAllAndPushAppState(gManager->findByName(StateNames.at(StandByStateId)));
						}
					}
				}
				...
				{
				//	mRegionGuide->hide();
					//if (gManager->getActiveStateStack()->back()->m_Index==GuideStateId)
					//{
					if (gManager->getActiveStateStack()->back()->m_Index!=GuideStateId)
						gManager->popAllAndPushAppState(gManager->findByName(StateNames.at(GuideStateId)));
					else
					{
						...
						for (unsigned int i=0;i<candidateUsers.size();i++)
						{
							...
							{
								OgreFramework::getSingletonPtr()->m_pLog->logMessage("An Active User Found, User:" + StringConverter::toString(candidateUsers.at(i)));
								interestedUser=0;
					...
					//}
					//if (interestedUser==0 && activeUser==0)
					//{
					//	unsigned int i;
					//	for (i=0;i<candidateUsers.size();i++)
					//	{
					//		if (checkInterestRate(candidateUsers.at(i)))//Standing time can be checked here as well
					//		{
					//			OgreFramework::getSingletonPtr()->m_pLog->logMessage("An Interested User Found:"+Ogre::StringConverter::toString(candidateUsers.at(i)));
					//			interestedUser=candidateUsers.at(i);
					//			if (gManager->getActiveStateStack()->back()->m_Index!=GuideStateId)
					//				gManager->popAllAndPushAppState(gManager->findByName(StateNames.at(GuideStateId)));
					//			break;
					//		}
					//	}
					//	if(i==candidateUsers.size())
					//	{
					//		if (gManager->getActiveStateStack()->back()->m_Index!=InterestStateId)
					//		{
					//			mRegionGuide->hide();
					//			gRegionFlash->hide();
					//			OgreFramework::getSingletonPtr()->m_pLog->logMessage("No Interested User Found");
					//				gManager->popAllAndPushAppState(gManager->findByName(StateNames.at(InterestStateId)));
					//		}
					//	}
					...
					//	else 
					//	{
					//		OgreFramework::getSingletonPtr()->m_pLog->logMessage("Interested User Cannot Be Located, resetting..");
					//		interestedUser=0;
					//	}
					//}
				...
				if (activeUser!=0)//If there is an active user, which is using the AdVivum
				{
					if (checkUserCOM(activeUser))//If the user is still in the region
					{
						...
						if (deleteUser)
						{
							OgreFramework::getSingletonPtr()->m_pLog->logMessage("Active User Recovered");
							deleteUser=false;
							elapsedTime=0;
						}
						...
						//m_HandTracker->initROI(This->activeUser);
							...
							if (!lookingForPose)
							{
								OgreFramework::getSingletonPtr()->m_pLog->logMessage("Initiating Pose Search for Active User");
	...
	//						changeCandidateID(activeUser);
							OgreFramework::getSingletonPtr()->m_pLog->logMessage("Initiating Tracking for Active User");
							m_UserGenerator.GetSkeletonCap().StartTracking(activeUser);
							trackingUser=true;
						}
					//	#ifndef USE_CALIBRATION_FILE
						...
						{
							OgreFramework::getSingletonPtr()->m_pLog->logMessage("Setting Up Hand For Active User");
							m_HandTracker->initROI(activeUser);
						}
					//	#endif
						else
					...
					else//If the active user stepped out of the active region
					{
						...
						{
							OgreFramework::getSingletonPtr()->m_pLog->logMessage("Active user stepped out too far,will be deleted");
						...
						else
						{
							if (deleteUser)
							{
								OgreFramework::getSingletonPtr()->m_pLog->logMessage("Deleting active user");
								trackingUser=false;		//Lose the current user
								deleteUser=false;
								mRightHandState=HAND_UNKNOWN;
								mLeftHandState=HAND_UNKNOWN;
								OgreFramework::getSingletonPtr()->m_pLog->logMessage("Updating Statistics");
								mStatistics->lostUser();
								elapsedTime=0;
								m_UserGenerator.GetSkeletonCap().Reset(activeUser);	
								mRegionGuide->hide();
								gRegionFlash->hide();
								changeCandidateID(0);	//This clears the hand trackerd
								OgreFramework::getSingletonPtr()->m_pLog->logMessage("Deleted Active User");
							}
							else if (m_UserGenerator.GetSkeletonCap().IsTracking(activeUser))// If the user was being tracked
							{
								...
								OgreFramework::getSingletonPtr()->m_pLog->logMessage("User stepped out of the active region, initiating 3 second countdown before deletion");
							...
							{
								OgreFramework::getSingletonPtr()->m_pLog->logMessage("User was not calibrated, losing the user immediately");
	...
	void KinectController::updateRelativeHandCoordinates()
	{
		...
		if (m_UserGenerator.GetSkeletonCap().IsTracking(activeUser) )
		{
			...
			float x=handPos.position.X-(torsoPos.position.X-headToTorso*0.2);
			float y=handPos.position.Y-(headPos.position.Y-headToTorso*0.0);
			...
			y=handPos.position.Y-(headPos.position.Y-headToTorso*0.0);
			x=x/(2.0*headToTorso);
			y=y/(headToTorso*1.9);
			ppLeft.X=x;
			ppLeft.Y=y;
		...
		//Speed update
		unsigned int tempTime=OgreFramework::getSingleton().m_pTimer->getMilliseconds();
	...
	void KinectController::createRTT(Ogre::Root* ogreRoot,OgreBites::SdkTrayManager* mTrayMgr)
	{
		...
		mPlaneEnt->setMaterialName(gManager->getControlOverlay("RegionGuide")->getMaterialName());
