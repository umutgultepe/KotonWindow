...
FlashControl::FlashControl(const std::string& _name, Viewport* viewport, int _width, int _height, const Hikari::Position& position, unsigned short zOrder) : IFlashControl()
{
	name=_name;
	site=0;
	handler=0;
	...
	okayToDelete=false;
	isDraggable=true;
	isIgnoringTransparent=true;
	transThreshold=0.04;
	lastDirtyWidth = 0;
	lastDirtyHeight = 0;

	renderBuffer = new Hikari::Impl::RenderBuffer(_width, _height);
	
	createControl();
	createMaterial();

	overlay = new OgreViewportOverlay(name, viewport, width, height, position, materialName, zOrder);
	if(compensateNPOT)
		overlay->getPanel()->setUV(0, 0, (Real)width/(Real)texWidth, (Real)height/(Real)texHeight);
}

FlashControl::FlashControl(const std::string& _name, int _width, int _height)	
{
	name = _name;
	width= _width;
	height = _height;
	...
	isTransparent=false;
	okayToDelete=false;
 	isIgnoringTransparent=false;
	transThreshold=0;

	renderBuffer = new Hikari::Impl::RenderBuffer(width, height);
	createControl();
	createMaterial();
}
...
{
	site = new Hikari::Impl::FlashSite();
	site->AddRef();	
	site->Init(this, NULL);
	
	HMODULE flashLib = Hikari::HikariManager::getSingleton().flashLib;
	...
	handler = new Hikari::Impl::FlashHandler();
	handler->AddRef();	
	handler->Init(this);
...
void FlashControl::createMaterial()
{
	...
	texture = TextureManager::getSingletonPtr()->createManual(
	...
	if(overlay)
	{
		overlay->getPanel()->setMaterialName(materialName);
		if(compensateNPOT)
			overlay->getPanel()->setUV(0, 0, (Real)width/(Real)texWidth, (Real)height/(Real)texHeight);
	}
}

void FlashControl::load(const Ogre::String& movieFilename, Hikari::HikariManager* hikariMgr)
{
	std::string moviePath = hikariMgr->basePath + movieFilename;
...
void FlashControl::setQuality(short renderQuality)
{
	...
	case Hikari::RQ_BEST:
		...
		break;
	case Hikari::RQ_AUTOLOW:
		flashInterface->PutQuality2("autolow");
		break;
	case Hikari::RQ_AUTOHIGH:
...
void FlashControl::bind(const std::wstring& funcName, const Hikari::FlashDelegate& callback)
{
...
Hikari::FlashValue FlashControl::callFunction(const std::wstring& funcName, const Hikari::Arguments& args)
{
	BSTR returnVal = 0;
	HRESULT result = flashInterface->raw_CallFunction(_bstr_t(Hikari::Impl::serializeInvocation(funcName, args).c_str()), &returnVal);
	...
	return Hikari::Impl::deserializeValue((wchar_t*)returnVal);
}
...
{
	Hikari::HikariManager::getSingleton().focusControl(0, 0, this);
}
...
bool FlashControl::isPointOverMe(int screenX, int screenY)
{
	...
	if(overlay->isWithinBounds(screenX, screenY))
	{
		if(isTransparent && isIgnoringTransparent)
			return renderBuffer->getBuffer()[overlay->getRelativeY(screenY)*renderBuffer->rowSpan+overlay->getRelativeX(screenX)*4+3] > transThreshold * 255;
		else
			return true;
	}
...
void FlashControl::handleFlashCall(const std::wstring& xmlString)
{
	...
	if(!Hikari::Impl::deserializeInvocation(xmlString, funcName, args))
		return;

	DelegateIter i = delegateMap.find(funcName);
	if(i != delegateMap.end())
	{
		Hikari::FlashValue retval = i->second(this, args);
		flashInterface->SetReturnValue(Hikari::Impl::serializeValue(retval).c_str());
	}
}
